...
FSUBB = InlineKeyboardMarkup(
    ...
    if m.sender_chat:
        return await m.reply_text("☹️ [you're](tg://settings) an __Anonymous Admin__ !\n\nᗚ revert back to user account from admin rights.")
    try:
        aing = await c.get_me()
    except Exception as e:
    ...
    if a.status != "administrator":
        await m.reply_text(
            f"💡 To use me, I need to be an **Administrator** with the following **permissions**:\n\nᗚ ❌ __Delete messages__\nᗚ ❌ __Add users__\nᗚ ❌ __Manage video chat__\n\nData is **updated** automatically after you **promote me**"
        )
        return
    ...
    try:
        ...
        if b.status == "kicked":
            await m.reply_text(
                f"@{ASSISTANT_NAME} **is banned in group😑** {m.chat.title}\n\nᗚ **unban the userbot first if you want to use this bot.😑**"
            )
            return
    except UserNotParticipant:
        if m.chat.username:
    ...
    if replied:
        if replied.video or replied.document:
            ...
            if len(m.command) < 2:
                await m.reply(
                    "⭕️**usage**: /vplay [music name/yt url/vudio file]"
                )
            else:
